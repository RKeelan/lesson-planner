...
export default function matchLayout(
  ...
  if (layoutName === undefined) {
    ...
    // Check if the determined layout exists in the presentation
    // If not, fall back to the first available layout (for template presentations)
    const availableLayouts = presentation.layouts
      ?.map(l => l.layoutProperties?.name)
      .filter((name): name is string => Boolean(name));
    if (availableLayouts && !availableLayouts.includes(layoutName)) {
      if (availableLayouts.length > 0) {
        layoutName = availableLayouts[0];
      }
    }
  }
  if (!layoutName) {
    throw new Error('No valid layout found for slide');
  }
